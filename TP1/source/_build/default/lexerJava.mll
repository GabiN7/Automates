...
(* Analyseur lexicale : expression reguliere { action CaML } *)
rule lexer = parse
...
(* Nombres entiers : A COMPLETER *)
  | ('0' | (['1' - '9'] decimal*)) as texte   { (ENTIER (int_of_string texte)) }
  | ("0b" ('0'|'1')*) as texte { (BINAIRE (int_of_string texte)) }
(* Nombres flottants : A COMPLETER *)
  | (decimal+ "." decimal+) as texte     { (FLOTTANT (float_of_string texte)) }
